...
public partial class MenuConstructor : Control
{
	...
	private void IrABatalla()
	{
		...
		// Si no hay cartas en el mazo, ir igual (Campo1 usará mazo por defecto)
		if (escenas.Count == 0)
		{
			GD.Print("[Constructor] Sin cartas → usando mazo por defecto");
			string rutaBat = !string.IsNullOrEmpty(RutaBatalla) ? RutaBatalla : "res://escenas/gameplay/campo_1.tscn";
			GetTree().ChangeSceneToFile(rutaBat);
			return;
		}
	...
	private void ActualizarContador()
	{
		...
		{
			// Nunca deshabilitar - si no hay cartas usa el mazo por defecto
